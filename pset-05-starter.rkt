...
(define G2 (make-game (cons (make-ball 10 20 3 4)
...
(define (main lob)
  (big-bang lob
    (on-draw   render)   ;Game -> Image
    (on-tick   game-with-next-balls)     ;Game -> Game
...
(check-expect (render G0)
              (place-image PADDLE (game-paddle G0) PADDLE-CTR-Y MTS))
(check-expect (render G2)
              (place-image PADDLE (game-paddle G2) PADDLE-CTR-Y
                           (render-game-balls (game-balls G2))))
(check-expect (render G1)
              (place-image PADDLE (game-paddle G1) PADDLE-CTR-Y
                           (render-game-balls (game-balls G1))))
(check-expect (render (make-game (cons B1 (cons (make-ball 10 10 1 2)
                                                (cons (make-ball 30 30 1 -2)
                                                (cons (make-ball 40 40 1 -2)
                                                      empty)))) 30))
              (place-image PADDLE 30 PADDLE-CTR-Y
                           (render-game-balls (cons B1
                                                    (cons (make-ball 10 10 1 2)
                                                    (cons (make-ball 30 30 1 -2)
                                                    (cons (make-ball 40 40 1 -2)
                                                          empty)))))))
...
(define (render g)
  (place-image PADDLE (game-paddle g) PADDLE-CTR-Y
               (render-game-balls (game-balls g))))


(@htdf render-game-balls)
(@signature ListOfBall -> Image)
;; render the list of balls in the game
(check-expect (render-game-balls empty) MTS)
(check-expect (render-game-balls (cons (make-ball 10 20 3 4)
                                       (cons (make-ball 30 40 1 2)
                                             empty)))
...
; (define (render-game-balls lob) MTS) stub

(@template-origin ListOfBall)

(@template
 (define (render-game-balls lob)
   (cond [(empty? lob) (...)]
         [else
          (... (fn-for-ball (first lob))
               (render-game-balls (rest lob)))])))
...
(@signature Game -> Game)
;; produce the game-with-next-balls gamestate with proper LOB and Paddle pos.
(check-expect (game-with-next-balls G0) G0)
(check-expect (game-with-next-balls (make-game
                                     (cons (make-ball (+ LEF 1) TOP 3 -4)
                                           (cons (make-ball 200 100 3 4)
                                                 empty)) 205))
              (make-game (cons (next-ball (make-ball (+ LEF 1) TOP 3 -4))
                               (cons (next-ball (make-ball 200 100 3 4))
                                     empty)) 205))
(check-expect (game-with-next-balls (make-game (cons (make-ball LEF TOP 3 -4)
                                                     (cons
                                                      (make-ball LEF BOT 3 -4)
                                                           (cons
                                                            (make-ball
                                                             RIG TOP 3 -4)
                                                                 empty))) 200))
              (make-game (cons (next-ball (make-ball LEF TOP 3 -4))
                               (cons (next-ball (make-ball LEF BOT 3 -4))
                                     (cons (next-ball (make-ball RIG TOP 3 -4))
                                           empty))) 200))
(check-expect (game-with-next-balls (make-game (cons (make-ball LEF TOP 3 -4)
                                                     (cons
                                                      (make-ball LEF BOT 3 -4)
              ...
              (make-game (cons (next-ball (make-ball LEF TOP 3 -4))
                               (cons (next-ball (make-ball LEF BOT 3 -4))
                                     (cons
                                      (next-ball
                                       (make-ball LEF PADDLE-CTR-Y 3 -4))
                                           empty))) RIG))

; (define (next lob) empty) ;stub
...
(define (game-with-next-balls g)
  (make-game (next-game-balls (game-with-caught-balls (game-balls g)
                                                      (game-paddle g)))
             (game-paddle g)))
...
;; remove entries from a list that touch the paddle, otherwise return the list
(check-expect (game-with-caught-balls empty 10) empty)
(check-expect (game-with-caught-balls (cons (make-ball 100 PADDLE-CTR-Y 1 2)
                                             (cons (make-ball 200 200 1 2)
                                                   empty)) 100)
              (cons (make-ball 200 200 1 2) empty))

(check-expect (game-with-caught-balls  (cons (make-ball 100 TOP 1 2)
                                       (cons (make-ball LEF PADDLE-CTR-Y 1 2)
                                                             empty)) 100)
              (cons (make-ball 100 TOP 1 2)
                    (cons (make-ball LEF PADDLE-CTR-Y 1 2) empty))) 
                                      
                                       
; (define (game-with-caught-balls lob) empty) ;stub
 ...
 (define (game-with-caught-balls lob p)
   (cond [(empty? lob) (...)]
         [else
          (... p
               (fn-for-ball (first lob))
               (game-with-caught-balls (rest lob)))])))
  ...
  (cond [(empty? lob) empty]
        [else
         (if (touch-paddle? (first lob) p)
             ...
             lob)]))
;; RNR a list where all balls in the list/(rest lob) are not touching the paddle
...
(define (game-with-caught-balls g)
  (cond
    [(empty? (game-balls g)) empty]
    [(touch-paddle? (first (game-balls g)) (game-paddle g))
     (rest (game-balls g))]
    [else (game-balls g)]))



(@htdf next-game-balls)
(@signature ListOfBall -> ListOfBall)
;; produce the next list of balls for the game
(check-expect (next-game-balls empty) empty)
(check-expect (next-game-balls (cons (make-ball (+ LEF 1) TOP 3 -4)
                                     (cons (make-ball 200 100 3 4)
                                           empty)))
              (cons (next-ball (make-ball (+ LEF 1) TOP 3 -4))
                    (cons (next-ball (make-ball 200 100 3 4))
                          empty)))
#;
(define (next-game-ball lob) empty) ;stub

(@template-origin ListOfBall)

(@template
 (define (next-game-balls lob)
   (cond [(empty? lob) (...)]
         [else
          (... (fn-for-ball (first lob))
               (next-game-balls (rest lob)))])))
...
(check-random (handle-mouse (make-game empty 205) 100 200 "button-down")
              (make-game (cons (make-ball 100 200 (- 5 (random 11))
                                          (- 5 (random 11))) empty) 205))
(check-random (handle-mouse (make-game LOB2 205) 100 200 "button-down")
              (make-game (handle-game-mouse LOB2 100 200 "button-down") 205))
(check-random (handle-mouse (make-game (cons B1
                                       (cons (make-ball 10 10 -2 0) empty)) 205)
                            100 200 "button-up") 
              (make-game (handle-game-mouse (cons B1
                                       (cons (make-ball 10 10 -2 0) empty))
                                            100
                                            200
                                            "button-up") 205))
...
(@template
 (define (handle-game-mouse lob x y me)
   (cond [(mouse=? me "button-down") (... lob x y)]
         [else
          (... lob x y)])))

(define (handle-mouse g x y me)
  (make-game (handle-game-mouse (game-balls g) x y me) (game-paddle g)))
...
(check-random (handle-game-mouse empty 100 200 "button-down")
              (cons (make-ball 100 200 (- 5 (random 11)) (- 5 (random 11)))
                    empty))
(check-random (handle-game-mouse (cons (make-ball 10 20 -3 3) empty)
                                 300 100 "button-down")
              (cons (make-ball 300 100 (- 5 (random 11)) (- 5 (random 11)))
                    (cons (make-ball 10 20 -3 3) empty)))
(check-random (handle-game-mouse empty 100 200 "button-up") empty)
(check-random (handle-game-mouse (cons (make-ball 10 20 -3 3) empty)
                                 100 200 "button-up")
              (cons (make-ball 10 20 -3 3) empty))
#;
...
;; clear all balls if space is pressed, left arrow, right arrow move the paddle
(check-expect (handle-key (make-game (cons (make-ball (/ WIDTH 2) (/ HEIGHT 2)
                                                      2 4)
                                           (cons (make-ball (+ TOP 2) (+ LEF 5)
                                                            3 2)
                                                 empty)) 205) " ")
              (make-game empty 205))
(check-expect (handle-key (make-game (cons (make-ball (/ WIDTH 2) (/ HEIGHT 2)
                                           ...
                                           (cons (make-ball (+ TOP 2) (+ LEF 5)
                                                            3 2)
                                                 empty)) 205) "c")
              (make-game (cons (make-ball (/ WIDTH 2) (/ HEIGHT 2)
                                          2 4)
...
(@template   
 (define (handle-key g ke)
   (cond [(key=? ke " ") (... (game-balls g))]
         [else 
          (... (game-paddle g))])))

(define (handle-key g ke)
  (cond [(key=? ke " ") (make-game empty (game-paddle g))]
        [(key=? ke "left") (make-game (game-balls g) (- (game-paddle g)
                                                        PADDLE-MOVE-PER-KEY))]
        [(key=? ke "right") (make-game (game-balls g) (+ (game-paddle g)
                                                         PADDLE-MOVE-PER-KEY))]
        [else g]))
