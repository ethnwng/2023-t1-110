...
;; purpose is to compare the heights of two images and return the larger image
;(define (taller-image i1 i2) empty-image) ;stub


(check-expect (taller-image (rectangle 20 40 "outline" "black")
                            (rectangle 20 50 "solid" "blue"))
              (rectangle 20 50 "solid" "blue"))

(check-expect (taller-image (rectangle 50 100 "outline" "purple")
                            (rectangle 80 50 "solid" "white"))
              (rectangle 50 100 "outline" "purple"))

(check-expect (taller-image (rectangle 50 50 "outline" "white")
                            (rectangle 60 50 "solid" "pink"))
              (rectangle 50 50 "outline" "white"))
...
(define (taller-image i1 i2)
  (if (< (image-height i1) (image-height i2)) i2 i1))
