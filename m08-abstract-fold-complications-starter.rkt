...
;; fold functions do not give
;; the original compount data values

;; when you need the original data
;; ---> have to use old approach of
;; copying template and editing

#|
(@template-origin use-abstract-fn)

(define (courses-w-credits c n)
  (local [(define (c1 num cre rmr)
            (if (>= cre n)
                (cons <THE CURR COURSE> rmr) ; no way to get the current course
                rmr))]
