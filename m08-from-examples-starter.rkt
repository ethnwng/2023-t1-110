...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname m08-from-examples-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
...
;; produce list with only those elements of lon for which p produces true
...
(@htdf map2)
;(@signature ___ ___ -> ___) ;(define (map2 fn lon))
;(@signature ___ ___ -> (listof __)) ;(empty? lon) empty)
;(@signature ___ (listof __) -> (listof __)) ;(empty? lon) empty)
;(@signature (__ -> __) (listof __) -> (listof __));fn is some function
;(@signature (X -> ___) (listof X) -> (listof __));the input of the fn must be
; the same as the items in the output list
(@signature (X -> Y) (listof X) -> (listof Y)) ;the output of the function
; must be the same as the output of the items in the list
