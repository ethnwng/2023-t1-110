...
;(bitmap/url
;"https://
;edx-course-spdx-kiczales.s3.amazonaws.com/HTC/lab/lab-07-yes-no-tree.png")
...
(define YNT1 (make-yntree "Does it gobble?"
                          ...
                          (make-yntree "Does it say 'Nevermore'?"
                                       "Raven"
                                       "Eagle")))
(define YNT2 (make-yntree "Is it a mammal"
                          (make-yntree
...
;; produce true if there exists an answer after n questions
...
(check-expect (contains-n-path? YNT2 3) true)

; (define (contains-n-path? yntree n) false)

#|
    n                      0              (add1 Natural)
ynt

String                    true [1]        false[1]

(make-yntree String       false[  (or(contains-n-path? (sub1 n) (yntree-y ynt))
                                      (contains-n-path? (sub1 n)(yntree-n ynt))
             YesNoTree
             YesNoTree)
|#
...
;; produce a list of matches for the given list of players
(check-expect (set-matches LOPL1 LOPL2)
              (list M0 M1
                    (make-match "Sharon" "Elena")
                    (make-match "Aleksandra" "Anastasia")))
(check-expect (set-matches empty LOPL2) empty)
(check-expect (set-matches LOPL1 empty) empty)
(check-expect (set-matches empty empty) empty)

#|
     lop2                           empty             (cons Player ListOfPlayer)
lop1

empty                               empty             empty

(cons Player ListOfPlayer)          empty             (cons (make-match
...
(@template-origin 2-one-of)
(define (set-matches lop1 lop2)
  (cond [(or (empty? lop1) (empty? lop2)) empty]
        [else
         (cons (make-match (first lop1) (first lop2))
               (set-matches (rest lop1) (rest lop2)))]))
...
(@template-origin Person ListOfPerson encapsulated try-catch)
(define (find n p)
