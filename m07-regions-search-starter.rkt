...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname m07-regions-search-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require 2htdp/image)
(require spd/tags)
...
(check-expect (find-region "apple" I4) false)

(@template-origin Region ListOfRegion encapsulated try-catch)

(define (find-region s r)
  (local [(define (find-region--region r)
            (cond [(leaf? r) (if (string=? s (leaf-label r))
                                 r
                                 false)]
                  [else
                   (find-region--lor (inner-subs r))]))

          (define (find-region--lor lor)
            (cond [(empty? lor) false]
                  [else
                   (local [(define try (find-region--region (first lor)))]
                     (if (not (false? try))
                         try
                         (find-region--lor (rest lor))))]))]

    (find-region--region r)))
