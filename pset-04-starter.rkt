...
;; - empty (empty list)
;; - Ball  (ref)
;; - (cons Ball ListOfBall) (self-ref)
;; interp. List of Balls
(define LOB1 empty)
(define LOB2 (cons B1 empty))
(define LOB3 (cons B1 (cons (make-ball (/ WIDTH 2) (/ HEIGHT 2) 4 -3) empty)))

(@dd-template-rules one-of
                    atomic-distinct ; empty
                    compound  ; cons
                    ref ; Ball (first lob)
                    self-ref) ; Rest of List (rest lob)
...
(define (main lob)
  (big-bang lob
    ...
    ;ListOfBall Integer Integer MouseEvent -> ListOfBall
    (on-key    handle-key)))   ;ListOfBall KeyEvent -> ListOfBall
            
(@htdf render-balls)
(@signature ListOfBall -> Image)
;; place balls on image at appropriate x, y coordinates
(check-expect (render-balls (cons (make-ball LEF TOP -2 -3)
                                  empty))
              (place-image BALL LEF TOP MTS))
(check-expect (render-balls empty) MTS)
(check-expect (render-balls (cons (make-ball LEF TOP 2 3)
                                  (cons (make-ball RIG BOT -2 -3)
                                        empty)))
              (place-image BALL LEF TOP (place-image BALL RIG BOT MTS)))
...
(check-expect (render-ball (make-ball 20 30 3 3) MTS)
              (place-image BALL 20 30 MTS))
(check-expect (render-ball (make-ball (- WIDTH 4) (- HEIGHT 5) -2 -3)
                           empty-image)
              (place-image BALL (- WIDTH 4) (- HEIGHT 5) empty-image))

; (define (render-ball b img) img)
...
(@htdf next-balls)
(@signature ListOfBall -> ListOfBall)
;; produce the next balls at the position with x-speed and y-speed
(check-expect (next-balls empty) empty)
(check-expect (next-balls (cons (make-ball LEF TOP 3 2) empty))
              (cons (make-ball (+ LEF 3) (+ TOP 2) 3 2) empty))
(check-expect (next-balls (cons (make-ball LEF TOP 3 2)
                                (cons (make-ball RIG BOT -3 -2) empty)))
              (cons (make-ball (+ LEF 3) (+ TOP 2) 3 2)
                    (cons (make-ball (+ RIG -3) (+ BOT -2) -3 -2) empty)))
...
;; place a ball at x y on mouse pressed down
...
(check-random (handle-mouse empty 100 200 "button-down")
              (cons (make-ball 100 200 (- 5 (random 11)) (- 5 (random 11)))
                    empty))
(check-random (handle-mouse empty 100 200 "drag")
              empty)
(check-random (handle-mouse empty 50 50 "button-up")
              empty)
(check-random (handle-mouse (cons (make-ball 10 10 2 2)
                                  (cons (make-ball 15 15 4 4) empty))
                            100 100 "button-down")
              (cons (make-ball 100 100 (- 5 (random 11)) (- 5 (random 11)))
               (cons (make-ball 10 10 2 2)
                                  (cons (make-ball 15 15 4 4) empty))))
...
;; when space is pressed clear all balls
(check-expect (handle-key (cons B1 (cons (make-ball 50 50 -2 -3) empty)) " ")
              empty)
(check-expect (handle-key (cons B1 (cons (make-ball 50 50 -2 -3) empty)) "w")
              (cons B1 (cons (make-ball 50 50 -2 -3) empty)))

; (define (handle-key k event) k) ;stub
