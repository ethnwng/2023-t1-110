...
;; produce true if nothing in the first list is not prefix of second
...
; (define (prefix=? lst1 lst2) false) ;stub

(@template-origin 2-one-of)
;two arguements that have one of type, and will be "going" into them


;; creating a table with lst1 lst2 as the param, and the rows and colums
...
(define (prefix=? lst1 lst2)
  (cond [(empty? lst1) true] ;[1]
        [(empty? lst2) false] ;[2]
        [else
         (and (string=? (first lst1) (first lst2)) ;[3]
